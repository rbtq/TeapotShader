...
void main() {
    ...
    vec3 ambient = materialDef.Ka * lightDef.La;
   ...
   vec3 D = materialDef.Kd*lightDef.Ld* max(dot(N,L), 0.0); // For transformations
   D = clamp(D, 0.0, 1.0);
   ...
   vec3 specular = clamp(materialDef.Ks * lightDef.Ls * pow(viewAngle,lightDef.Lsp),0.0,1.0);
